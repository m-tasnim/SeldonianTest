...
def getData():
    K = np.loadtxt(open("data.csv", "rb"), delimiter=",")
    X = K[:, 1:10]
    Y = K[:, 10]
    T = K[0]
    return X, Y, T
...
def predict(theta, x):
    return theta[0] + theta[1] * x[0] + theta[2] * x[1] + theta[3] * x[2] + theta[4] * x[3] + theta[5] * x[4] + theta[6] * x[5] + theta[6] * x[5] + theta[7] * x[6] + theta[8] * x[7] + theta[9] * x[8]
...
def gHat1(theta, X, Y):
    ...
    for i in range(n):
        ...
        res[i] = np.mean((prediction - Y[i]) * (prediction - Y[i]))  # Compute the squared error for the i-th data point
...
def leastSq(X, Y):
    ...
    theta1 = reg.coef_[0][0]  # Gets theta1, the slope coefficient
    theta2 = reg.coef_[0][1]
    theta3 = reg.coef_[0][2]
    theta4 = reg.coef_[0][3]
    theta5 = reg.coef_[0][4]
    theta6 = reg.coef_[0][5]
    theta7 = reg.coef_[0][6]
    theta8 = reg.coef_[0][7]
    theta9 = reg.coef_[0][8]
    return np.array([theta0, theta1, theta2, theta3, theta4, theta5, theta6, theta7, theta8, theta9])
...
if __name__ == "__main__":
    ...
    (X, Y, T) = getData()  # Generate the data
